...
def train(model_path):
    ...
    train_dataset = SpeakerDatasetTIMITPreprocessed(hp.data.train_path,phase="train")
    ...
    if hp.model.type=='LSTM':
        embedder_net = LSTMEmbedder(hp).to(device) 
    elif hp.model.type=='ecapatdnn':
        embedder_net = ECAPA_TDNN(1024,hp).to(device) 
