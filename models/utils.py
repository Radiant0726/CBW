...
def get_centroids(embeddings):
    centroids = embeddings.mean(dim=-2)
    return centroids
...
def get_cossim_nosame(embeddings, centroids):
    # number of utterances per speaker
    num_utterances = embeddings.shape[1] # try_times

    # flatten the embeddings and utterance centroids to just utterance,
    # so we can do cosine similarity

    embeddings_flat = embeddings.reshape( # [bz*try_times,dim]
        embeddings.shape[0] * num_utterances, 
        -1
    )
    # the cosine distance between utterance and the associated centroids
    ...
    # R (centroids) where L has each utterance repeated sequentially for all
    # comparisons and R has the entire centroids frame repeated for each utterance
    centroids_expand = centroids.repeat((num_utterances * embeddings.shape[0], 1)) # [bz*try_times*bz,dim]
    embeddings_expand = embeddings_flat.unsqueeze(1).repeat(1, embeddings.shape[0], 1) # [bz*try_times,bz,dim]
    embeddings_expand = embeddings_expand.view( # [bz*try_times*bz,dim]
        embeddings_expand.shape[0] * embeddings_expand.shape[1],
        embeddings_expand.shape[-1]
    )
    cos_diff = F.cosine_similarity(embeddings_expand, centroids_expand) # [bz*try_times*bz]
    cos_diff = cos_diff.view( #
        embeddings.size(0), # bz
        num_utterances, # try_times
        centroids.size(0) # bz
    )
    # assign the cosine distance for same speakers to the proper idx
    cos_diff = cos_diff + 1e-6
...
def get_cossim_nosame_times(embeddings, centroids):
    # number of utterances per speaker
    num_utterances = embeddings.shape[1] # try_times
    # embeddings.shape[0] = 1
    bz = centroids.shape[0]  # [bz,N,dim]
    N = centroids.shape[1] 
    embeddings_flat = embeddings.reshape( # [1*try_times,dim]
        embeddings.shape[0] * num_utterances, 
        -1
    )
    centroids_expand = centroids.repeat((num_utterances,1, 1)) # [bz*try_times,N,dim]
    centroids_expand = centroids_expand.reshape(bz*num_utterances*N,-1)  # [bz*try_times*N,dim]
    embeddings_expand = embeddings_flat.unsqueeze(0).unsqueeze(2).repeat(bz, 1, N, 1) # [bz,try_times,N,dim]
    embeddings_expand = embeddings_expand.view( # [bz*try_times*N,dim]
        bz*num_utterances*N,
        embeddings_expand.shape[-1]
    )
    cos_diff = F.cosine_similarity(embeddings_expand, centroids_expand) # [bz*try_times*N]
    cos_diff = cos_diff.view( #
        centroids.size(0), # bz
        num_utterances, # try_times
        N, # N
