...
def train(model_path):
    ...
    train_dataset = SpeakerDatasetTIMITPreprocessed(hp.poison.poison_train_path,phase="train")
    ...
    for e in range(hp.train.epochs):
        ...
        if hp.train.poison_checkpoint_dir is not None and (e + 1) % hp.train.checkpoint_interval == 0:
            ...
            ckpt_model_path = os.path.join(hp.train.poison_checkpoint_dir, ckpt_model_filename)
            torch.save(embedder_net.state_dict(), ckpt_model_path)
            print("saved.",ckpt_model_filename)
            embedder_net.to(device).train()

    #save model
    embedder_net.eval().cpu()
    save_model_filename = "final_epoch_" + str(e + 1) + "_batch_id_" + str(batch_id + 1) + ".model"
    save_model_path = os.path.join(hp.train.poison_checkpoint_dir, save_model_filename)
